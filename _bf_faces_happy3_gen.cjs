/* ═══════════════════════════════════════════════════════════════════════════
   _bf_faces_happy3_gen.cjs — 补齐三位顾客的「满意(happy)」表情头像

   背景：art/icons/faces/ 里 fang_happy / lu_happy 已由 _bf_assets2_gen.cjs 切好，
   但 stud / office / uncle 三位**只有 calm / urgent**，于是「订单全部拿到」时
   breakfast.js 的 drawCustomerFace 走了 fallback（happy → calm → 矢量）。

   本批上级 agent 用 Lovart 生成了一张 **3 列 × 1 行** 的满意表情表
   （学生红色鸭舌帽 / 女白领 / 胖大爷，三人开心笑，纯白底）：
       art/lovart_cca01cd7ba70.png   2172×724

   ⚠ 本脚本**不重写**任何切片 / 去白底算法：它 require _bf_assets2_gen.cjs，
   ...
   运行：node _bf_faces_happy3_gen.cjs [源图路径]
   产物：art/icons/faces/{stud,office,uncle}_happy.png  +  art/_faces_happy3_report.json
   ═══════════════════════════════════════════════════════════════════════════ */
"use strict";
const fs = require("fs");
const path = require("path");
const A2 = require("./_bf_assets2_gen.cjs");        // ← 复用既有切片实现，不重写
...
/* 与既有 faces 组（_bf_assets2_gen.cjs 的 GROUPS[1]）逐项对齐：
...
const report = {
  ...
  note: "三位顾客「满意」头像的切片参数（由 _bf_faces_happy3_gen.cjs 调用 _bf_assets2_gen.cjs 的 sliceGrid 自动求出，未写死）",
  reuse: "sliceGrid() / whiteToAlphaCfg() / fitCanvas() 全部来自 _bf_assets2_gen.cjs，未重写",
