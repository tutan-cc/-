/* ═══════════════════════════════════════════════════════════════════════════
   _bf_shots.cjs — 出图：用自写软件光栅化器回放 breakfast.js 真实发出的 Canvas2D 指令
   ...
   流程：① Node 真跑 breakfast.js（真动画循环）→ 记录式 Canvas2D
        ② 形状交给 _bf_raster.cjs 软件光栅化
        ③ 文字交给 __textHook 收集 → _bf_text.ps1 用 System.Drawing + 系统字体（微软雅黑）
           把真汉字合成回 PNG —— 出图里的中文是清楚的系统字体，不是 5×7 点阵/方块。
   运行：node _bf_shots.cjs          （会顺带调用 pwsh 合成文字；失败会提示手动跑）
        pwsh -File _bf_text.ps1     （只重跑文字合成）
   ═══════════════════════════════════════════════════════════════════════════ */
const fs = require("fs");
...
fs.writeFileSync(path.join(OUT, "tests", "breakfast-shots.json"),
  JSON.stringify({
    ...
    note: "非浏览器截图：把 breakfast.js 真发出的 Canvas2D 指令在 Node 里光栅化；文字由 _bf_text.ps1 用系统字体（微软雅黑）合成",
...
if (!BITMAP_TEXT) {
  ...
  const ps = path.join(OUT, "_bf_text.ps1");
  ...
  if (r.error || r.status !== 0) {
    console.log("[文字合成] 调用 PowerShell 失败(" + ((r.error && r.error.message) || ("exit " + r.status)) +
                ") → 请手动执行：powershell -File _bf_text.ps1（或 node _bf_shots.cjs --bitmap-text 用点阵字兜底）");
  }
}
