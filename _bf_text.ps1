# _bf_text.ps1 — 用 System.Drawing + 系统字体（Microsoft YaHei）把真汉字合成回光栅化出来的 PNG
...
# 用法：powershell -File _bf_text.ps1   （读 tests\bf_shots_text.json，原地覆盖 测试截图\*.png）
param(
  [string]$Manifest = (Join-Path $PSScriptRoot "tests\bf_shots_text.json")
)
...
if (-not (Test-Path $Manifest)) { throw "找不到清单：$Manifest（先跑 node _bf_shots.cjs）" }
