/* ═══════════════════════════════════════════════════════════════════════════
   浏览器验收：早餐店 · 拼手速（_e2e_breakfast.js）
...
(async () => {
  ...
  if (ch.ok) {
    ...
    extraNote += "；注意：Trident 降级下「侧栏入口 / 剧情分支 / 结算面板 DOM」无法覆盖（HTA 里没有 index.html 的页面壳），" +
      "只验证模块本体；这部分由 _e2e_bf_headless.cjs（Node + 记录式 Canvas2D + 可泵 rAF，66 项）补上。";
  }
  const rasterShots = (() => { try { return fs.readdirSync(SHOT_DIR).filter(f => /^bf_.*\.png$/.test(f)); } catch (e) { return []; } })();
  extraNote += rasterShots.length
    ? "；测试截图/ 下的 bf_game.png / bf_pass.png / bf_fail.png 由 _bf_shots.cjs 用自写软件光栅化器回放同一份 breakfast.js 的真实 Canvas2D 指令生成（**不是浏览器截图**：本沙箱 Chrome/Edge 均 mojo platform_channel 0x5 无法启动、mshta 落盘被拦）。"
    : "；未产出 PNG 截图。";
  const res = {
