/* ═══════════════════════════════════════════════════════════════════════════
   _e2e_mj_system.js — 麻将系统（赌注 / 自由局 / 打法 / 邀约 / 剧情后果 / 口碑 / 情报）浏览器实测
   运行：node _e2e_mj_system.js
   ...
   模式说明（本机沙箱事实）：
     ...
     · 出图不在本脚本里做：沙箱里 mshta 的屏幕拷贝同一个进程超过 ~3 次就挂，
       四张验收图由 `node _mj_shots.cjs` 用独立进程 + System.Drawing 单独产出。

   产物：tests/mj-system-results.json
   ═══════════════════════════════════════════════════════════════════════════ */
const fs = require("fs");
const path = require("path");
const L = require("./_mj_probe_lib.cjs");
...
/* 中文 → JS \uXXXX 转义：直接用公共底座 _mj_probe_lib.cjs 的实现（HTA 文件保持纯 ASCII） */
const U = L.U;
...
(async () => {
  ...
  console.log("[出图] 四张验收图由 `node _mj_shots.cjs` 单独产出（本沙箱 mshta 同一进程屏幕拷贝超过 ~3 次会挂）");
  const checks = [], errors = [], info = {};
  let note = "Chrome(headless CDP)/Edge 在本沙箱均无法启动（mojo 命名管道被拒），降级用 mshta(Trident) 真实渲染同一份 mahjong.js（生产代码，非复刻）；出图由 `node _mj_shots.cjs` 用独立进程 + System.Drawing 单独产出（沙箱里同一 mshta 进程屏幕拷贝超过 ~3 次会挂）。";
  if (!tri.ok) {
  ...
  /* 出图由 `node _mj_shots.cjs` 负责（每个 mshta 进程只允许约 3 次屏幕拷贝，故与断言分开跑） */
