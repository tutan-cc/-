...
(function (root) {
  ...
  function drawTileBack(g, x, y, w, h, o) {
    ...
    g.restore();
    /* 本批素材：深蓝斜纹牌背贴图（等比塞满牌背矩形，居中裁切）。
       贴图不可用 → 回退下面的程序化斜纹（原样保留，一处没删）。 */
    var tbi = artOf("tile_back");
    if (tbi) {
      var inw = tbi.naturalWidth || 0, inh = tbi.naturalHeight || 0;
      if (inw > 0 && inh > 0) {
        var arT = inw / inh, arB = w / h, swT, shT;
        if (arT > arB) { shT = inh; swT = inh * arB; } else { swT = inw; shT = inw / arB; }
        g.save();
        rr(g, x, y, w, h, r); g.clip();
        if (o.light) g.globalAlpha = 1; else g.globalAlpha = 0.94;
        g.drawImage(tbi, (inw - swT) / 2, (inh - shT) / 2, swT, shT, x, y, w, h);
        g.restore();
        G.tileBackTex = (G.tileBackTex || 0) + 1;
      }
    } else {
    var grd = g.createLinearGradient(x, y, x, y + h);
