/* ═══════════════════════════════════════════════════════════════════════════
   ...
   说明：本沙箱起不了 Chrome/Edge，最终验收图 bf_game_bg2.png / mj_bg.png 由
        _bf_shots2.cjs 出（那两张要真跑 breakfast.js / mahjong.js）。
   ═══════════════════════════════════════════════════════════════════════════ */
"use strict";
...
try {
  execFileSync("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
    path.join(OUT, "_bf_text.ps1"), "-Manifest", path.join(OUT, "tests", "bf_assets2_text.json")],
    { cwd: OUT, stdio: ["ignore", "inherit", "inherit"] });
} catch (e) {
  console.log("⚠ 中文合成没跑成（" + (e.message || e) + "）→ 请手动执行：powershell -File _bf_text.ps1 -Manifest tests\\bf_assets2_text.json");
}
