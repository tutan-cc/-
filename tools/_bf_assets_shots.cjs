/* ═══════════════════════════════════════════════════════════════════════════
   ...
   证据链同上一轮：本沙箱起不了 Chrome/Edge，所以
     breakfast.js 真跑（无头 vm）→ 它发出的每条 Canvas2D 指令进 _bf_raster.cjs
...
try {
  execFileSync("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
    path.join(OUT, "_bf_text.ps1"), "-Manifest", path.join(OUT, "tests", "bf_assets_text.json")],
    { cwd: OUT, stdio: ["ignore", "inherit", "inherit"] });
} catch (e) {
  console.log("⚠ 中文合成没跑成（" + (e.message || e) + "）→ 请手动执行：powershell -File _bf_text.ps1 -Manifest tests\\bf_assets_text.json");
}
