/* ═══════════════════════════════════════════════════════════════════════════
   tools/bf/assets/gen2.js — 第二批 Lovart 素材切片/裁切（零依赖，可重复运行）
   ...
   用法：node tools/bf/assets/gen2.js [--only=plates,faces,mj,bg,kitchen2]
   ═══════════════════════════════════════════════════════════════════════════ */
"use strict";
...
/* ═══ 组定义（行优先 = 素材里的排列顺序）═══
   源文件名不写死：由命令行按**生成顺序**传入，脚本自己查表：
     node tools/bf/assets/gen2.js <盘面六宫格> <早餐店背景v2> <顾客头像六宫格> [麻将包间背景] [麻将道具九宫格]
...
const report = { at: new Date().toISOString(), view: VIEW,
  note: "第二批 Lovart 素材的全部切片/裁切参数（tools/bf/assets/gen2.js 自动求出，未写死）",
