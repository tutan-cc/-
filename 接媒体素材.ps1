<#
  接媒体素材.ps1 —— 把「媒体素材」目录挂进本仓库（Windows 目录联接，零拷贝 / 零代码改动）
  ...
  用法：
    ...
    powershell -ExecutionPolicy Bypass -File .\接媒体素材.ps1 -MediaDir "D:\某处\媒体素材"
...
请把媒体素材包解压到本仓库同级目录，命名为「重生2-媒体素材」，使其结构为：
  $parent\重生2-媒体素材\video\  (35 个 mp4 + poster\ 35 张 jpg)
  $parent\重生2-媒体素材\audio\  (vo\ + vo_real\ + mj\)
...
if ($manifest -and $manifest.files) {
  ...
  if ($tp -gt 0) {
    Write-Host "  提示：$tp 个文件来源为 thirdparty（授权未核实，仅供本地试验）；换成自拍/AI 生成后，重跑 打包媒体素材.ps1 更新清单即可。" -ForegroundColor DarkGray
  }
}
